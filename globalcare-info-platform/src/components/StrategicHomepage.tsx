...
const content = {
  EN: {
    ...
    sectorsLabel: "Sector Focus", sectorsTitle: "WHERE EXECUTION CREATES VALUE", sectorsIntro: "Six sectors where GCI's market, supply chain and operating capabilities intersect.",
  ...
  ZH: {
    ...
    sectorsLabel: "重点行业", sectorsTitle: "执行创造价值的领域", sectorsIntro: "聚焦 GCI 市场、供应链与运营能力交汇的六个行业。",
  ...
  AR: {
    ...
    sectorsLabel: "القطاعات", sectorsTitle: "حيث يصنع التنفيذ القيمة", sectorsIntro: "ستة قطاعات تلتقي فيها قدرات GCI السوقية والتشغيلية وسلسلة الإمداد.",
...
const sectors = [
  { name: "Construction & Infrastructure", icon: Construction }, { name: "Automotive & New Energy", icon: CarFront },
  { name: "Industrial & Manufacturing", icon: Factory }, { name: "Real Estate & Living", icon: Building2 },
  { name: "Trade & Logistics", icon: Ship }, { name: "Technology & AI", icon: CircuitBoard }
];
...
export default function StrategicHomepage({ lang }: { lang: Lang }) {
  ...
  return <div dir={isRtl ? "rtl" : "ltr"}>
    ...
    <section id="sectors" className="py-20 md:py-24 bg-[#070b18] border-b border-brand-gold-500/10"><div className="max-w-7xl mx-auto px-6"><SectionHead label={c.sectorsLabel} title={c.sectorsTitle} intro={c.sectorsIntro}/><div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">{sectors.map((s,i)=>{const Icon=s.icon;return <div key={s.name} className="flex items-center gap-4 rounded-xl border border-brand-gold-500/10 bg-[#030611]/70 p-5 hover:border-brand-gold-500/30 transition-colors"><span className="text-[10px] font-mono text-brand-gold-500/55">0{i+1}</span><Icon className="w-5 h-5 text-brand-gold-400 shrink-0"/><span className="text-sm text-brand-gold-100/90">{s.name}</span></div>})}</div></div></section>
