...
export default function DailyBriefingPage({ lang }: { lang: Lang }) {
  ...
  return (
    <main className="min-h-[70vh] bg-[#030611] py-12 md:py-16">
      <div className="max-w-5xl mx-auto px-6">
        <a href="/" className="inline-flex items-center gap-2 text-sm text-brand-gold-400"><ArrowLeft className="h-4 w-4" />{back}</a>
        <div className="mt-8 flex flex-col md:flex-row md:items-end md:justify-between gap-5 border-b border-brand-gold-500/15 pb-6">
          <div>
            <p className="text-xs font-mono tracking-[0.18em] text-brand-gold-400">DAILY BRIEFING</p>
            <h1 className="mt-2 text-3xl md:text-5xl font-serif font-bold text-brand-gold-100">{title}</h1>
        ...
        <div className="mt-7 space-y-4">
          {data?.items.map((item, index) => (
            <article key={item.id} className="rounded-xl border border-brand-gold-500/12 bg-[#071022] p-5 md:p-6">
              ...
              {item.source_url && <a href={item.source_url} target="_blank" rel="noreferrer" className="mt-5 inline-flex items-center gap-1.5 text-xs text-brand-gold-400 hover:text-brand-gold-300">{item.source_name || "Source"}<ExternalLink className="h-3.5 w-3.5" /></a>}
            </article>
          ))}
        </div>
      </div>
